...
<form action="Cart.action" method="post">
	<table>
		...
		<c:forEach var="cartItem" items="${actionBean.cart.cartItems}">
			<tr>
				<td><a
					href="Catalog.action?viewItem=&itemId=${cartItem.item.itemId}">
				...
				<td><fmt:formatNumber value="${cartItem.total}"
					pattern="$#,##0.00" /></td>
				<td><a class="Button"
					href="Cart.action?removeItemFromCart=&cartItem=${cartItem.item.itemId}">
            	Remove
            </a></td>
			</tr>
		</c:forEach>
		<tr>
			<td colspan="7">Sub Total: <fmt:formatNumber
				value="${actionBean.cart.subTotal}" pattern="$#,##0.00" /> <input
				type="submit" name="updateCartQuantities" value="Update Cart" /></td>
			<td>&nbsp;</td>
		</tr>
	</table>

</form> <c:if test="${actionBean.cart.numberOfItems > 0}">
	<a class="Button"
		href="Order.action?newOrderForm=">
      	Proceed to Checkout
      </a>
</c:if></div>
